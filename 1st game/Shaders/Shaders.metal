...
static constexpr constant short kNumLayers = 8;
 ...
 )
{
    ...
    for (short i = kNumLayers - 1; i >= 0; i-=2)
        {
            half4 layerColor = fragmentValues.colors[i-1];
            
            half firstDepth = fragmentValues.depths[i];
            half secondDepth = fragmentValues.depths[i-1];
            
            //layerColor = (firstDepth == half(INFINITY)) ? fragmentValues[i1]
            firstDepth = (firstDepth == half(INFINITY)) ? 0.0 : firstDepth;
            
            half differenceOfDepths = (secondDepth-firstDepth);
            
            //layerColor.w += (secondDepth!=half(INFINITY)) ? differenceOfDepths : 0.h;
            layerColor.xyz += (secondDepth!=half(INFINITY)) ? differenceOfDepths * 0.001h : 0.h;
            
            //layerColor.w = (layerColor.w > 1.h) ? max(layerColor.w, 1.1h) : layerColor.w;
