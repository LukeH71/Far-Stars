...
// `[MTLRenderCommandEncoder setVertexBuffer:offset:atIndex]` requires that buffer offsets be 256 bytes aligned for buffers using the constant address space and 16 bytes aligned for buffers using the device address space. The sample uses the device address space for the `actorParams` parameter of the shaders and uses the `set[Vertex|Framgment:offset:` methods to iterate through `ActorParams` structures. So it aligns each element of `_actorParamsBuffers` by 16 bytes.
#define BufferOffsetAlign 16

#define MaxBuffersInFlight 3

#define AspectRatio 1.6f
